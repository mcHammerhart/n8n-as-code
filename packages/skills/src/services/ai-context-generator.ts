...
export class AiContextGenerator {
  ...
  private getAgentsContent(
    ...
    return [
      ...
      `   - \`${cliCmd} test <id> --prod\`: Calls the **production** webhook URL — **use this after \`workflow activate\`**. Works immediately, no manual arm needed.`,
      `   - \`${cliCmd} test <id>\`: Calls the test-mode webhook URL — only if the workflow is NOT activated. Requires a manual arm in the n8n editor first ("Listen for test event").`,
      `   - **⚠️ MANDATORY RULE: If \`workflow activate <id>\` was run at any point in this session, ALWAYS use \`test <id> --prod\`. Never call bare \`test <id>\` after activation — the test URL is not armed and will return an error.**`,
      ...
      `\`\`\`bash`,
      `# After workflow activate — ALWAYS use --prod:`,
      `${cliCmd} test <workflowId> --prod`,
      ``,
      `# Without activation (draft/inactive) — requires manual arm in n8n editor first:`,
      `${cliCmd} test <workflowId>`,
      `\`\`\``,
      `- **⚠️ MANDATORY: After \`workflow activate\`, ALWAYS pass \`--prod\`. The test-mode URL requires a manual arm and will fail without it. The production URL works immediately post-activation.**`,
