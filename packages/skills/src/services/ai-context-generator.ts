...
export class AiContextGenerator {
  ...
  private getWorkspaceBootstrapLines(cliCmd: string): string[] {
    return [
      ...
      `- If the user has already provided the n8n host and API key, prefer running \`${cliCmd} init --yes\` non-interactively instead of asking them to type through the wizard manually.`,
      `- If required values are still missing, stop and ask for the missing host, API key, or project selector before continuing.`,
      `- Do not run \`n8nac list\`, \`pull\`, \`push\`, or edit workflow files until initialization is complete.`,
      `- Never write \`n8nac-config.json\` by hand. Initialization must go through \`${cliCmd} init\` so credentials and AI context stay consistent.`,
      ...
      `- Preferred non-interactive form: \`${cliCmd} init --yes --host <url> --api-key <key> [--project-id <id>|--project-name <name>|--project-index <n>]\``,
      `- \`${cliCmd} init --yes\` also reads \`N8N_HOST\` and \`N8N_API_KEY\` from the environment when available.`,
      ...
      `2. If missing and you already have the required values, run \`${cliCmd} init --yes\` with the appropriate flags.`,
      `3. If values are missing, ask the user for them and then run \`${cliCmd} init --yes\` or ask the user to run \`${cliCmd} init\` manually.`,
