...
export class AiContextGenerator {
  ...
  private getAgentsContent(
    ...
    return [
      ...
      `   - **⚠️ MANDATORY RULE: ALWAYS run \`workflow activate <id>\` before testing. ALWAYS pass \`--prod\`. Never call bare \`test <id>\` — the test URL is not armed and will return an error.**`,
      ...
      `\`\`\``,
      `- **⚠️ MANDATORY: ALWAYS activate the workflow first, then test with \`--prod\`. Never use bare \`test <id>\` — the test-mode URL requires a manual arm in the n8n editor and will fail without it.**`,
